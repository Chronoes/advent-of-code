...
784	232	363	316	336	666	711	430	192	867	628	57	222	575	622	234"
  ...
  def solve(:first, input) do
    input
    |> convert_input
    |> Stream.map(fn list -> Enum.max(list) - Enum.min(list) end)
    |> Enum.sum()
  end
  ...
  def solve(:second, input) do
    input
    |> convert_input
    |> Stream.map(fn list -> list |> Enum.sort() end)
    |> Stream.map(&find_even_div/1)
    |> Enum.sum()
  end
  ...
  # Part 2
  def find_even_div([]), do: 0
  def find_even_div([_a]), do: 0

  def find_even_div([a | tail]) do
    result = find_even_div(a, tail)
  ...
  def find_even_div(_a, []), do: 0
  def find_even_div(a, [b | _tail]) when rem(b, a) == 0, do: div(b, a)
  def find_even_div(a, [_b | tail]), do: find_even_div(a, tail)
end
